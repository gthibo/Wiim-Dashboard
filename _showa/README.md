...
**Post Round 44 — USB output cap fix (upstream `e61b4b9`):** Ported the upstream USB DAC output cap (limits USB output selection to the DAC when a USB device is attached). Fork port `3ec1233`, merged as `61e3a43` (2026-09-14).

**Post Round 44 — USB chain follow-on (upstream `b27b2f0`):** Ported the upstream `fetchUsbDac` → `fetchSoundCard` rename plus `availableOutputs` addition in `commands.ts` + `snapshot.ts`. Merged as `30198ef` (2026-09-14). Unblocked Round 44a's remaining `c68c950` scope.

**Round 44a:** c68c950 data-layer subset extracted via multi-model pipeline (Opus design → Kimi K3 spec → Qwen3.8 Max audit → Opus adjudication → deepseek-v4-pro exec). 6 files, +91/−14: `parse.ts` (`decodeTimingSec` + UDisk guard), `upnp.ts` (`actualQuality` + STATION/RADIO mode keys `"12"`/`"13"`), `commands.ts` (`actualQuality` thread + trailing-space `albumArtURI ` fallback + `un_known` + `pickPic`), `now-playing-info.ts` (`INTERNAL_VENDOR_NAMES += "udisklocal"` + 5-arg `inferAudioFormat`), `capabilities.ts` (`plm_support` false-positive pruning), `snapshot.ts` (`actualQuality` wire around FORK DELTA). Hi-res tier verified live on Amazon UHD (`actualQuality="UHD"` → `qualityHiRes=true`). Merged as `0100854` (2026-09-15).

**Round 44b:** Snapshot batch + `c68c950` UI-layer subset. 6 files, +231/−108: `constants.ts` (`MEDIA_SOURCE_KEYS` export), `parse.ts` (`bea000f` `deriveSource` prioritised match), `upnp.ts` (`0472b2b` DIDL-Lite `<res>` fallback), `ARCHITECTURE.md` (upnp.ts row + `fetchSoundCard` drift), `now-playing-card.tsx` (`isMediaSource`/`isPhysicalInput` refactor + hide-timeline-when-stopped, both trees). Multi-model pipeline via kimi-k3 exec. Merged as `bb94ae4` (2026-09-16).

**Post Round 44b — radio-transport hotfix:** Extended `isRadio` in `now-playing-card.tsx` with vendor-string branches (`vtuner`, `customradio`, `newtunein`) — fixes phantom skip/shuffle/repeat buttons on WiiM firmware that lands radio streams on `sourceMode: "10"` (not the 12/13 upstream assumed). Single 2-file dual-write, +28/−2. Merged as `dd96540` (2026-09-16).

**Round 44d:** Trusted artwork hosts + `isPlexArtUrl` retirement. Adopted upstream `7d99a97`'s opt-in allowlist as the fork's LAN media-server SSRF policy, retiring the fork-specific `isPlexArtUrl` URL-shape shim in the same commit. 7 files, +275/−33: `db/settings.ts` (`artHosts` key + `getArtHosts`/`setArtHosts`/`MAX_ART_HOSTS = 8`), `wiim/client.ts` (SSRF helpers `isSensitiveIp`/`normaliseArtHost`/`artHostKey`/`getInsecureAgent`/`warnRefusedArtHost` + `wiimFetchRaw` allowlist rewrite + shim retirement), `client/hooks.ts` (`SettingsResponse.artHosts`), `settings/route.ts` (`PatchSchema.artHosts` + `validateArtHosts` DNS-rebinding guard + GET/PATCH wiring + `BAD_ART_HOST`), `art/route.ts` + `preset-art/route.ts` (`allowHosts: getArtHosts()` on both, with slave→master redirect and Round 39 URL-keyed cache preserved), `settings-view.tsx` (new `ArtworkHosts` panel with add/remove UI — single tree; `_showa/components/settings/` does not exist, so no dual-write). Multi-model pipeline via deepseek-v4-pro exec. Smoke-passed live with `192.168.1.107:32400` as Greg's Plex host (a `192.168.1.7:32400` typo was caught cleanly by the allowlist gate — refused-host warning fired with the exact `host:port` to add, validating the design). Merged as `2105774` (2026-09-16).

**Round 44c:** Upstream EQ cluster (`3107749` + `9ca026e` + `e7b55f2`) ported through the multi-model pipeline: Opus design → deepseek-v4-pro spec-write → Hermes/Qwen3.8 Max audit → Opus adjudication → **deepseek-v4.1-flash exec (first round on flash tier, clean pass)**. 12 files, +183/−71: `getAcousticCapability` probe + `AcousticCapability` type + `capabilities.acoustic` field (parallel-probe extended 5→6 in `capabilities.ts`); LP/HP filter types + new `PEQ_GAIN_INDEPENDENT_MODES` set + "—" dB display for gain-independent bands; `peqLettersFrom` letter-derivation infra reading band count from device response instead of hardcoding 10 (`PEQ_LETTERS` = a–l wire-accepted, new `PEQ_LETTERS_BASELINE` = a–j always-rendered, `PEQ_LETTERS_ALL` retired); `bandColor()` re-anchored on `PEQ_LETTERS_BASELINE.indexOf()` (D6); Off-band visual repair — selective per-column dim + `title` hint + "—" dB when Off, replacing upstream's whole-row `opacity-45`. Opus adjudication folded 3 corrections into Task 2.1 (auditor's Issue A + B + Opus-added Minor C on `BAND_COLORS` docstring). Merged as `bf8d757` (2026-09-17). Ultra smoke passed; Pro smoke exposed a firmware quirk (12 rows visible instead of expected 10) corrected in 44e.

**Round 44e:** Band-count clip fix following 44c smoke discovery. Both Ultra (fw 5.2.8x) and Pro (2026-09 firmware) answer `EQGetLV2SourceBandEx` with 12 letters a–l and persist writes to k/l on re-read, but an audible A/B test on the Ultra (L @ Peak/250Hz/+9.5 dB silent vs D at identical params clearly audible) proved only a–j are wired to the DSP chain. `GetAcousticCapability` doesn't expose functional band count (probed on both devices — `PEQ` subtree carries only `Version` + `Filters` list). `ParametricPanel` now filters `bands` through `PEQ_LETTERS_BASELINE.includes()` — matches WiiM Home's own 10-row UI. Data-layer `peqLettersFrom` infra preserved for hypothetical future firmware that wires k/l live (drop the one filter in `ParametricPanel` and everything downstream handles it). 3 files, +19/−5: `eq-card.tsx` (both trees) + `docs/WIIM-API.md` paragraph rewrite documenting the empirical finding. Opus-direct exec (small scope, in-session follow-on). Merged as `eb8324a` (2026-09-17). The previously-tentative "Round 44e — `6c8b37d` in-place IP edit" candidate was retasked to this clip; the IP-edit candidate stays un-numbered until claimed.

**Round 44f:** In-place IP edit on the Devices settings page (`/devices`) — adopts upstream `6c8b37d` byte-for-byte (git blob `ac05cef1907b659f1ecb6a8af2721e42c05ad2e2`). Adds a pencil beside each device's `model · address` line that opens an input with save/cancel affordances mirroring the rename control; saving PATCHes the device with the new host and best-effort re-probes so capability chips reflect the box actually at that address. Solves the common failure mode where a stale DHCP lease turns the whole dashboard silent while the scrobbler logs EHOSTUNREACH every 15 s. Zero API/data-layer surface — the fork's `PATCH /api/devices/[id]` route already accepted `host` with `isPrivateHost` LAN validation (400 `FORBIDDEN_HOST`) and `getDeviceByHost` duplicate detection (409 `DUPLICATE`) from an earlier round. 1 file, +74/−3: `src/components/devices/device-manager.tsx` only (`_showa/components/devices/` mirror does not exist — same `src/`-only pattern as Round 44d's settings changes). Ground-truth checks pre-scope confirmed the file had zero divergence from upstream's `6c8b37d^` baseline and zero Showa design tokens, so upstream landed via clean cherry-pick with `CHANGELOG.md` (fork-excluded per policy) dropped as "keep ours". Opus-direct exec, no multi-model pipeline. Smoke-passed on Ultra: pencil visible, toggle/cancel/save all work, valid save persists and refreshes capability chips, `8.8.8.8` rejected with "Host must be a private/LAN address.", duplicate IP rejected with "Another device already uses this address.". Merged as `06ca97c` (2026-09-18).

**Round 44g:** Two items bundled — Devices relabel and IP pencil-edit in the dashboard Device column. (1) The Device column action tile previously read "Add Device" with a `Plus` icon, misleading since `/devices` is the full list-and-manage page. Relabeled to "Devices" with the `Server` icon; five comment references updated. `app-header.tsx` confirmed orphaned — not touched. (2) Added inline IP pencil-edit to the IP `InfoRow` inside `DeviceSection` in `source-output-panel.tsx`, adapting the 44f pencil pattern from `device-manager.tsx` to the dashboard Device column. Pencil beside the address opens a mono `Input` pre-filled with the current IP; Check/Enter PATCHes `/api/devices/${deviceId}` with the new host and best-effort POSTs `/refresh` to re-probe; X/Escape cancels; toast on validation errors. `device-info-card.tsx` confirmed orphaned — the actual IP row lives in `DeviceSection`, not in that file. 1 file + mirror (2 files total): `source-output-panel.tsx` src/ and `_showa/` trees. Dual-write: SHA256 byte-identical verified. Typecheck exit 0, lint exit 0 (baseline PEQ_RANGE warning only). Opus-direct exec. Smoke-passed on Ultra. Merged as `debe25a` (2026-09-18).
